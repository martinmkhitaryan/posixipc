...
def test_queue_ctor_rejected():
    with pytest.raises(TypeError):
        posixipc.Queue()


def test_queue_requires_depth_and_item_size():
    layout = posixipc.Layout()
    with pytest.raises(ValueError):
        layout.add(posixipc.Queue)
    with pytest.raises(ValueError):
        layout.add(posixipc.Queue, depth=4)
    with pytest.raises(TypeError):
        layout.add(posixipc.Queue, depth=4, item_size=8, on_owner_died=lambda m: None)
...
def test_same_depth_same_digest():
    a = posixipc.Layout()
    b = posixipc.Layout()
    a.add(posixipc.Queue, depth=8, item_size=16)
    b.add(posixipc.Queue, depth=8, item_size=16)
    assert a.digest == b.digest
    c = posixipc.Layout()
    c.add(posixipc.Queue, depth=8, item_size=32)
    assert c.digest != a.digest


def test_attach_mismatch_item_size(shm_name):
    layout, queue, region = _make(shm_name, depth=4, item_size=8)
    other = posixipc.Layout()
    other.add(posixipc.Queue, depth=4, item_size=16)
    try:
...
def test_killed_waiter_on_full_queue(shm_name):
    ...
    ctx = procs.spawn_ctx()
    ready = ctx.Event()
    proc = ctx.Process(target=procs.child_queue_block_put, args=(shm_name, 2, 8, ready))
    try:
...
def _queue_bytes(name, queue, region):
    return posixipc._posixipc._attach_slot(name, queue.slot + 4, posixipc._posixipc.KIND_BYTES, region.digest)


def _state_off():
    cl = posixipc._posixipc.CACHELINE_BYTES
    ctrl = 24
    return ((ctrl + cl - 1) // cl) * cl
